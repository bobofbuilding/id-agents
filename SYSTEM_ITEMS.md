...
34. `GET /events` — Team-scoped catch-up read over `event_log` (same auth/team resolution as `/remote` via `teamContextMiddleware` → `getTeam`). **Query:** `since` — exclusive seq cursor (non-negative integer, default `0`); `limit` — page size (positive integer, default `100`, max `1000`); `topics` — optional comma-separated filter; alias tokens (`query:terminal`, `task:status`, `agent:lifecycle`) expand via `TOPIC_ALIASES` to concrete topic names server-side (`agent:lifecycle` → `agent:started`, `agent:stopped`, `agent:rebuild`). **JSON body:** `events[]` (seq, team, topic, occurred_at, actor, subject, data), stable team `stream_id`, `next_seq`, unfiltered `latest_available_seq`, `cursor_reset` (plus `cursor_reset_reason: "ahead_of_log"` when a stale cursor is beyond the current team log), `replay_truncated`, `earliest_available_seq` — see `MANAGER-POLLING.md`
