...
export class SqliteQueriesRepo implements QueriesRepository {
  ...
  private async latestAuditHash(teamId: string, queryId?: string): Promise<string | null> {
    ...
    let exclude = '';
    if (queryId) {
      exclude = 'AND query_id <> ?';
      params.push(queryId);
    }
    const r = await this.db.query<{ metadata: string | null }>(
      `SELECT metadata
       FROM queries
       WHERE team_id = ? ${exclude}
       ORDER BY created DESC, query_id DESC
       LIMIT 1`,
      params,
    );
    const metadata = parseJsonObject(r.rows[0]?.metadata);
    const chain = metadata?.audit_chain;
  ...
  async create(
    ...
    await this.db.query(
      ...
      [teamId, queryId, agentId, hardened.prompt, created, sessionId ?? null, own.owner_kind, own.owner_id, stringifyJson(hardened.metadata)],
    );
  }
  ...
  async upsert(
    ...
    await this.db.query(
      ...
      [
        ...
        own.owner_kind,
        own.owner_id,
        stringifyJson(hardened.metadata),
      ],
    );
  }
